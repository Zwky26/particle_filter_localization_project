...
class LikelihoodFieldMeasurementUpdate(object):
    ...
    def robot_scan_received(self, data):
        ...
        for i in cardinal_directions_idxs:
            measurements.append(data.ranges[i])
        ...
        for particle in self.particle_cloud:
            ...
            for i in range (len(measurements)):
                if measurements[i] <= 3.5:
                    euler_angle = euler_from_quaternion([current_pose.orientation.x, current_pose.orientation.y, current_pose.orientation.z, current_pose.orientation.w])
                    cosine = math.cos(euler_angle[2] + cardinal_directions_idxs[i])
                    sine = math.sin(euler_angle[2] + cardinal_directions_idxs[i])
                    x_z = current_pose.position.x + (measurements[i] * cosine)
                    y_z = current_pose.position.y + (measurements[i] * sine)
